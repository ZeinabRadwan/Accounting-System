@php
    ...
    // Extract data from reportData - it's a paginated collection
    $suppliers = $reportData['data'] ?? [];
        ...
        <div style="position: relative;">
            @php
                ...
                foreach ($suppliers as $supplier) {
                    $purchaseDue = $supplier['supplier_due'] ?? 0;
                    $purchaseReturnDue = $supplier['supplier_return_due'] ?? 0;
                    $totalPurchaseDue += $purchaseDue;
                    $totalPurchaseReturnDue += $purchaseReturnDue;
                    $totalDue += ($purchaseDue + $purchaseReturnDue);
                }
            @endphp
            ...
            <div class="text-center mb-4" style="margin-bottom: 30px;">
                <h2 style="margin: 0 0 10px 0; color: {{ $colors['primary'] ?? '#1f2937' }}; font-size: {{ ($typography['headingSize'] ?? 24) }}px; font-weight: bold;">@lang('print.Supplier Payable Report')</h2>
            </div>
                ...
                <div class="table-responsive" style="overflow-x: auto;">
                    <table class="table table-bordered table-striped table-sm">
                        <thead>
                            <tr>
                                <th>@lang('print.#')</th>
                                <th>@lang('print.Supplier ID')</th>
                                <th>@lang('print.Name')</th>
                                <th>@lang('print.Contact Number')</th>
                                <th>@lang('print.Email')</th>
                                <th>@lang('print.Company Name')</th>
                                <th>@lang('print.Purchase Due')</th>
                                <th>@lang('print.Purchase Return Due')</th>
                        ...
                        <tbody>
                            @foreach($suppliers as $index => $supplier)
                                @php
                                    $purchaseDue = $supplier['supplier_due'] ?? 0;
                                    $purchaseReturnDue = $supplier['supplier_return_due'] ?? 0;
                                    $supplierTotalDue = $purchaseDue + $purchaseReturnDue;
                                @endphp
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    ...
                                    <td class="text-right">{{ formatPdfCurrency($purchaseDue) }}</td>
                                    <td class="text-right">{{ formatPdfCurrency($purchaseReturnDue) }}</td>
                                    <td class="text-right">{{ formatPdfCurrency($supplierTotalDue) }}</td>
                                    <td>{{ ($supplier['status'] ?? false) ? __('print.Active') : __('print.Inactive') }}</td>
                                </tr>
                            @endforeach
                            <tr class="table-info">
                                <td colspan="6" class="text-right"><strong>@lang('print.Total')</strong></td>
                                <td class="text-right"><strong>{{ formatPdfCurrency($totalPurchaseDue) }}</strong></td>
                                <td class="text-right"><strong>{{ formatPdfCurrency($totalPurchaseReturnDue) }}</strong></td>
                                <td class="text-right"><strong>{{ formatPdfCurrency($totalDue) }}</strong></td>
                                <td></td>
                            </tr>
                        </tbody>
                    </table>
            ...
            @else
                <div class="text-center mt-5" style="margin-top: 50px; text-align: center;">
                    <h4 style="color: {{ $colors['secondary'] ?? '#6b7280' }};">@lang('print.No supplier data found.')</h4>
                </div>
            @endif
        </div>
    </div>
