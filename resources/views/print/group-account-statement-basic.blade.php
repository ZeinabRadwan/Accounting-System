...
<body>
    ...
    <div class="header">
        ...
        <p class="{{ app()->getLocale() === 'ar' ? 'arabic-text' : '' }}">@lang('print.This report was generated on') {{ date('d-M-Y H:i:s') }}
        </p>
    </div>
        ...
        <div class="accounts-info">
            ...
            @foreach ($groupStatementData['chart_of_accounts'] as $account)
                <div style="margin-bottom: 10px;">
                    ...
                    <span class="{{ app()->getLocale() === 'ar' ? 'arabic-text' : '' }}">{{ $account['code'] ?? '' }} -
                        {{ $account['name'] ?? '' }}</span><br>
                    <strong class="{{ app()->getLocale() === 'ar' ? 'arabic-text' : '' }}">@lang('Account Type'):</strong>
                    <span
                        class="{{ app()->getLocale() === 'ar' ? 'arabic-text' : '' }}">{{ $account['type'] ?? '' }}</span>
                </div>
            @endforeach
        </div>
    @endif
