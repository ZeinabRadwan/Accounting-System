<div>
    ...
    >
        <x-ui.button variant="secondary" href="{{ route('admin.customers.index') }}">{{ __('Back') }}</x-ui.button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 mb-6">
        <x-ui.stat-card label="{{ __('Outstanding') }}" :value="'EGP '.number_format($statement['current_due'], 2)" icon="clipboard" tone="danger" />
        ...
        <x-ui.stat-card label="{{ __('Period Sales') }}" :value="'EGP '.number_format($statement['period_sales'], 2)" icon="shopping-cart" tone="slate" />
        <x-ui.stat-card label="{{ __('Period Payments') }}" :value="'EGP '.number_format($statement['period_payments'], 2)" icon="cash" tone="success" />
    </div>

    <h2 class="text-sm font-semibold text-slate-800 mb-2">{{ __('Invoices') }}</h2>
        ...
        <x-ui.table-body>
            @forelse ($statement['invoices'] as $invoice)
                <tr class="hover:bg-slate-50/80" wire:key="inv-{{ $invoice->id }}">
                    <x-ui.td>
                        <a class="font-medium text-primary-600 hover:underline" href="{{ route('admin.sales.index', ['invoice' => $invoice->id]) }}">
                    ...
                    <x-ui.td><x-ui.status-badge :status="$invoice->status" /></x-ui.td>
                </tr>
            @empty
                <tr><x-ui.td colspan="7" class="text-center text-slate-500 py-6">{{ __('No invoices in this period.') }}</x-ui.td></tr>
            @endforelse
        </x-ui.table-body>
    </x-ui.table>

    <h2 class="text-sm font-semibold text-slate-800 mb-2">{{ __('Payments / Collections') }}</h2>
    <x-ui.table class="mb-8">
        <x-ui.table-head>
        ...
        <x-ui.table-body>
            @forelse ($statement['payments'] as $payment)
                <tr class="hover:bg-slate-50/80" wire:key="pay-{{ $payment->id }}">
                    <x-ui.td class="whitespace-nowrap">{{ $payment->paid_at?->format('Y-m-d H:i:s') }}</x-ui.td>
                    <x-ui.td class="font-medium text-success-700">EGP {{ number_format((float) $payment->amount, 2) }}</x-ui.td>
                    ...
                    <x-ui.td class="text-sm">
                        @forelse ($payment->applications as $app)
                            <a class="text-primary-600 hover:underline" href="{{ route('admin.sales.index', ['invoice' => $app->sales_invoice_id]) }}">
                                {{ $app->invoice?->invoice_number ?? ('#'.$app->sales_invoice_id) }}
                            </a>
                        ...
                        @empty
                            ...
                            @endif
                        @endforelse
                        @if ((float) $payment->credit_portion > 0 && $payment->applications->isNotEmpty())
                            <span class="text-success-700"> · {{ __('Credit') }} {{ number_format((float) $payment->credit_portion, 2) }}</span>
                        @endif
                    </x-ui.td>
                    <x-ui.td class="text-slate-500">{{ $payment->notes ?: '—' }}</x-ui.td>
                </tr>
            @empty
                <tr><x-ui.td colspan="6" class="text-center text-slate-500 py-6">{{ __('No collections in this period.') }}</x-ui.td></tr>
            @endforelse
        </x-ui.table-body>
    </x-ui.table>
                ...
                <tr class="hover:bg-slate-50/80" wire:key="stmt-{{ $line->sort }}">
                    ...
                    <x-ui.td>
                        @php
                            ...
                            $typeLabel = match ($line->entry_type) {
                                'sale' => __('Invoice'),
                                'payment' => __('Collection'),
                                'pos_payment' => __('Sale payment'),
                                'return' => __('Return'),
                    ...
                    <x-ui.td>
                        @if (! empty($line->invoice_id))
                            <a class="text-primary-600 hover:underline" href="{{ route('admin.sales.index', ['invoice' => $line->invoice_id]) }}">{{ $line->reference }}</a>
                        @else
                            {{ $line->reference }}
                        @endif
                    </x-ui.td>
    ...
    <x-ui.modal name="record-payment" title="{{ __('Record Payment') }}" show-property="showPaymentForm" maxWidth="lg">
        <div class="space-y-4">
            ...
            @if ($pay_apply_mode === 'invoice')
                <x-ui.select label="{{ __('Invoice') }}" wire:model="pay_sales_invoice_id" :error="$errors->first('pay_sales_invoice_id')">
            ...
            <p class="text-xs text-slate-500">{{ __('Amounts above outstanding invoice balances are saved as customer credit / advance.') }}</p>
        </div>
        <x-slot:footer>
