        ...
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <a v-if="$can('purchase-edit') ||
                    $can('purchase-view') ||
                    $can('purchase-delete')
                  " @click="exportToPDF" href="#" v-tooltip="$t('Export to PDF')" class="btn export-pdf-btn"
                    title="Export to PDF">
            ...
            <GeneralTable
              ...
              :rows="itemsWithIndex"
              :loading="loading"
              :show-actions="$can('purchase-edit') || $can('purchase-view') || $can('purchase-delete')"
            >
              ...
              <template #cell-code="{ row }">
                ...
                <span v-else>{{ row.code | withPrefix(prefix) }}</span>
              </template>
                    ...
                    <ul>
                      <li v-if="$can('purchase-view') && row.due > 0 && row.status === 1">
                        <a href="#" @click.prevent="handleModal(row)">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"
                            fill="none">
                            <path d="M8 1L10.5 5.5L15.5 6.5L12 10L13 15L8 12.5L3 15L4 10L0.5 6.5L5.5 5.5L8 1Z"
                              fill="#10B981" />
                          </svg>
                          {{ $t('Add Payment?') }}
                        </a>
                      ...
                      <li v-if="$can('purchase-view')">
                        <router-link :to="{ name: 'purchases.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"
                            fill="none">
                            <path
                      ...
                      <li v-if="$can('purchase-edit')">
                        <router-link :to="{ name: 'purchases.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"
                            fill="none">
          ...
          <div class="row">
            <div class="form-group col-md-6">
              ...
              <input type="text" class="form-control" readonly
                :value="parseFloat(form.selectedPurchase.purchaseTotal).toFixed(2)" />
            </div>
            <div class="form-group col-md-6">
              <label for="due">{{ $t("Due") }}</label>
              <input type="text" class="form-control" readonly
                :value="parseFloat(form.selectedPurchase.due).toFixed(2)" />
            </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4">
              ...
              <input type="number" step="0.01" class="form-control"
                :class="{ 'is-invalid': form.errors.has('paidAmount') }" :placeholder="$t('Enter an amount')" required
                min="0.01" v-model="form.paidAmount" :max="form.selectedPurchase.due" />
...
export default {
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/purchases/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  ...
  methods: {
    ...
    // export table to PDF using html2pdf (hide action column)
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `purchases-${moment().format("YYYY-MM-DD")}.pdf`,
...
.purchases-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .purchases-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .purchases-table thead th:last-child {
...
.purchases-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.purchases-table .badge.bg-danger {
