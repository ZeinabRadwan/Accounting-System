                        ...
                        <div class="action-menu" v-if="openActionIndex === i" @click.stop>
                          <ul>
                            <li v-if="$can('purchase-view') && data.due > 0">
                              <a href="#" @click.prevent="handleModal(data)">{{ $t('Add Payment?') }}</a>
                            </li>
                            <li v-if="$can('purchase-view')">
                              <router-link :to="{ name: 'purchases.show', params: { slug: data.slug } }">{{ $t('View') }}</router-link>
                            </li>
                            <li v-if="$can('purchase-edit')">
                              <router-link :to="{ name: 'purchases.edit', params: { slug: data.slug } }">{{ $t('Edit') }}</router-link>
                            </li>
