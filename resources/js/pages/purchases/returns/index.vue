        ...
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('invoice-return-create')" :to="{ name: 'purchaseReturns.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            <GeneralTable
              ...
              :rows="purchaseReturnsWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
              :show-actions="showActions"
            >
              <template #cell-purReturnNo="{ row }">
                <router-link v-if="$can('invoice-return-view')" :to="{
                  name: 'purchaseReturns.show',
                  params: { slug: row.slug },
                }">
                  {{ row.purReturnNo | withPrefix(purReturnPrefix) }}
                </router-link>
                <span v-else>{{ row.purReturnNo | withPrefix(purReturnPrefix) }}</span>
              </template>
              <template #cell-purchaseNo="{ row }">
                {{ row.purchaseNo | withPrefix(purPrefix) }}
              </template>
              <template #cell-totalReturn="{ row }">
                {{ parseFloat(row.totalReturn).toFixed(2) }} <span class="saudi-riyal">ê</span>
              </template>
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('invoice-return-view')">
                        <router-link :to="{ name: 'purchaseReturns.show', params: { slug: row.slug } }">
...
export default {
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/purchase-returns/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
    showActions() {
    ...
    purchaseReturnColumns() {
      return [
        ...
        { key: "purReturnNo", label: this.$t("Return No") },
...
.purchase-returns-table thead th {
...
.purchase-returns-table thead tr {
  border: none !important;
}

.purchase-returns-table thead th:first-child {
  border-top-left-radius: 10px;
}

.purchase-returns-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .purchase-returns-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .purchase-returns-table thead th:last-child {
...
.purchase-returns-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.purchase-returns-table .badge.bg-danger {
