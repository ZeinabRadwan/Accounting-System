          ...
          <div class="card-body position-relative">
            <div class="row d-fex" style="justify-content: flex-end">
              ...
              <div class="col-xl-6 col-3 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('adjustment-create')" :to="{ name: 'adjustments.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            <div class="table-responsive table-custom mt-3" id="printMe">
              <table class="table adjustments-table">
                ...
                <tbody>
                  <template v-show="items.length" v-for="(data, i) in items">
                    <!-- If adjustment has products, show each product in a separate row -->
                    <template v-if="data.adjustmentProducts && data.adjustmentProducts.length > 0">
                      <tr v-for="(product, productIndex) in data.adjustmentProducts" :key="`${i}-${productIndex}`">
                        <!-- Show adjustment info only on first product row -->
                        <td v-if="productIndex === 0" :rowspan="data.adjustmentProducts.length">
                        ...
                        <td v-if="productIndex === 0 && ($can('adjustment-edit') ||
                            $can('adjustment-delete') ||
                            $can('adjustment-view'))" :rowspan="data.adjustmentProducts.length" class="text-right no-print">
                        ...
                        <div class="action-menu" v-if="openActionIndex === i">
                          ...
                          <ul>
                            <li v-if="$can('adjustment-view')">
                              <router-link :to="{ name: 'adjustments.show', params: { slug: data.slug } }">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                            ...
                            <li v-if="$can('adjustment-edit')">
                              <router-link :to="{ name: 'adjustments.edit', params: { slug: data.slug } }">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                    ...
                    <!-- Fallback for adjustments without products -->
                    <tr v-else>
                      <td>
                        <span v-if="pagination && pagination.current_page > 1">
                          {{
                      ...
                      <td colspan="4" class="text-center">{{ $t("No products") }}</td>
                      <td>
                        <span v-if="data.status === 1" class="badge bg-success">{{
                          $t("Active")
                        }}</span>
                      ...
                      <td v-if="$can('adjustment-edit') ||
                        ...
                        <div class="action-dropdown" :class="{ open: openActionIndex === i }">
                          <button type="button" class="action-icon-btn" :data-action-index="i" @click.stop="toggleAction(i)">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none">
                              <path d="M13.125 12.7858C13.125 13.0083 13.059 13.2258 12.9354 13.4108C12.8118 13.5958 12.6361 13.74 12.4305 13.8252C12.225 13.9103 11.9988 13.9326 11.7805 13.8892C11.5623 13.8458 11.3618 13.7387 11.2045 13.5813C11.0472 13.424 10.94 13.2235 10.8966 13.0053C10.8532 12.7871 10.8755 12.5609 10.9606 12.3553C11.0458 12.1497 11.19 11.974 11.375 11.8504C11.56 11.7268 11.7775 11.6608 12 11.6608C12.2984 11.6608 12.5845 11.7794 12.7955 11.9903C13.0065 12.2013 13.125 12.4875 13.125 12.7858ZM12 7.53583C12.2225 7.53583 12.44 7.46985 12.625 7.34623C12.81 7.22262 12.9542 7.04691 13.0394 6.84135C13.1245 6.63578 13.1468 6.40958 13.1034 6.19135C13.06 5.97312 12.9528 5.77267 12.7955 5.61533C12.6382 5.458 12.4377 5.35085 12.2195 5.30744C12.0012 5.26404 11.775 5.28632 11.5695 5.37146C11.3639 5.45661 11.1882 5.60081 11.0646 5.78581C10.941 5.97082 10.875 6.18832 10.875 6.41083C10.875 6.7092 10.9935 6.99534 11.2045 7.20632C11.4155 7.4173 11.7016 7.53583 12 7.53583ZM12 18.0358C11.7775 18.0358 11.56 18.1018 11.375 18.2254C11.19 18.349 11.0458 18.5247 10.9606 18.7303C11.0458 18.9359 11.8532 19.1621 10.8966 19.3803C10.94 19.5985 11.0472 19.799 11.2045 19.9563C11.3618 20.1137 11.5623 20.2208 11.7805 20.2642C11.9988 20.3076 12.225 20.2853 12.4305 20.2002C12.6361 20.115 12.8118 19.9708 12.9354 19.7858C13.059 19.6008 13.125 19.3833 13.125 19.1608C13.125 18.8625 13.0065 18.5763 12.7955 18.3653C12.5845 18.1544 12.2984 18.0358 12 18.0358Z" fill="#023033"/>
                            </svg>
                          </button>
                          <div class="action-menu" v-if="openActionIndex === i">
                            <div class="action-menu-header">
