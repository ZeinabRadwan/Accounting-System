      ...
      <div class="col-lg-12">
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('non-purchase-payment-create')" :to="{ name: 'nonPurchasePayments.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            <GeneralTable
              :columns="nonPurchasePaymentColumns"
              :rows="nonPurchasePaymentsWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
            ...
            >
              ...
              <template #cell-paymentType="{ row }">
                <span v-if="parseInt(row.type) === 1" class="badge bg-primary">{{ $t('Payment Sent') }}</span>
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('non-purchase-payment-edit')">
                        <router-link :to="{ name: 'nonPurchasePayments.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('non-purchase-payment-show') || this.$can('non-purchase-payment-edit') || this.$can('non-purchase-payment-delete');
    },
    nonPurchasePaymentColumns() {
      return [
    ...
    exportUrl() {
      ...
      return `/payments/suppliers/non-purchase/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `supplier-non-purchase-payments-${(new Date()).toISOString().slice(0,10)}.pdf`,
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("You will not be able to return to this! This will delete the non-purchase payment permanently."),
        type: "warning",
...
.non-purchase-payments-table {
  border-collapse: separate;
  border-spacing: 0;
}

.non-purchase-payments-table thead th {
...
.non-purchase-payments-table thead tr {
  border: none !important;
}

.non-purchase-payments-table thead th:first-child {
  border-top-left-radius: 10px;
}

.non-purchase-payments-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .non-purchase-payments-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .non-purchase-payments-table thead th:last-child {
...
.non-purchase-payments-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.non-purchase-payments-table .badge.bg-danger {
