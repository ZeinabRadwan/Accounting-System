        ...
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('purchase-payment-create')" :to="{ name: 'purchasePayments.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            <GeneralTable
              ...
              :rows="purchasePaymentsWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
            ...
            >
              <template #cell-purchaseNo="{ row }">
                <router-link v-if="row.purchase && $can('purchase-view')" :to="{
                  name: 'purchases.show',
                  params: { slug: row.purchase.slug },
                }">
                  {{ row.purchase.purchaseNo }}
                </router-link>
                <span v-else-if="row.purchase">{{ row.purchase.purchaseNo }}</span>
              </template>
              <template #cell-supplier="{ row }">
                <span v-if="row.supplier">{{ row.supplier.name }}</span>
              </template>
              <template #cell-total="{ row }">
                <span v-if="row.purchase">{{ row.purchase.purchaseTotal }} <span class="saudi-riyal">ê</span></span>
              </template>
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('purchase-payment-view')">
                        <router-link :to="{ name: 'purchasePayments.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('purchase-payment-edit')">
                        <router-link :to="{ name: 'purchasePayments.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("Purchase Payments") };
  },
  components: {
    DateRangePicker,
    GeneralTable,
  ...
  computed: {
    ...
    showActions() {
      return this.$can('purchase-payment-edit') || this.$can('purchase-payment-view') || this.$can('purchase-payment-delete');
    },
    purchasePaymentColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "purchaseNo", label: this.$t("Purchase No") },
        { key: "supplier", label: this.$t("Supplier") },
        { key: "total", label: this.$t("Total") },
        { key: "paidAmount", label: this.$t("Paid Amount") },
        { key: "account", label: this.$t("Account") },
        { key: "paymentDate", label: this.$t("Payment Date") },
        { key: "status", label: this.$t("Status") },
      ];
    },
    purchasePaymentsWithIndex() {
    ...
    exportUrl() {
      ...
      return `/payments/suppliers/purchase/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `supplier-purchase-payments-${(new Date()).toISOString().slice(0,10)}.pdf`,
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("You will not be able to return to this! This will delete the purchase payment permanently."),
        type: "warning",
...
.purchase-payments-table thead th {
...
.purchase-payments-table thead tr {
  border: none !important;
}

.purchase-payments-table thead th:first-child {
  border-top-left-radius: 10px;
}

.purchase-payments-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .purchase-payments-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .purchase-payments-table thead th:last-child {
...
.purchase-payments-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.purchase-payments-table .badge.bg-danger {
