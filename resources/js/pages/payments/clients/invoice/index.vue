          ...
          <div class="card-body position-relative">
            ...
            >
              <template #cell-invoiceNo="{ row }">
                <router-link v-if="row.invoice && $can('invoice-view')" :to="{
                  name: 'invoices.show',
                  params: { slug: row.invoice.slug },
                }">
                  {{ row.invoice.invoiceNo | withPrefix(invoicePrefix) }}
                </router-link>
                <span v-else-if="row.invoice">{{ row.invoice.invoiceNo | withPrefix(invoicePrefix) }}</span>
              </template>
              <template #cell-client="{ row }">
                {{ row.client ? row.client.name : '' }}
              </template>
              <template #cell-total="{ row }">
                {{ row.invoice ? row.invoice.invoiceTotal : '' }} <span class="saudi-riyal">ê</span>
              </template>
              ...
              <template #cell-status="{ row }">
                ...
                <span v-else-if="row.status === 2" class="badge bg-danger">{{ $t("Cancelled") }}</span>
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('invoice-payment-view')">
                        <router-link :to="{ name: 'invoicePayments.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="row.attachment">
                        <a href="#" @click.prevent="viewAttachment(row.attachment)">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('invoice-payment-edit') && row.status !== 2">
                        <router-link :to="{ name: 'invoicePayments.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('invoice-payment-delete') && row.status === 1">
                        <a href="#" @click.prevent="cancelPayment(row.slug)">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                            <path d="M12 4L4 12M4 4L12 12" stroke="#F59E0B" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                          </svg>
                          {{ $t('Cancel Payment') }}
                        </a>
                      </li>
                      <li v-if="$can('invoice-payment-delete') && row.status === 0">
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("Invoice Payments") };
  },
  components: {
    DateRangePicker,
    GeneralTable,
  ...
  computed: {
    ...
    showActions() {
      return this.$can('invoice-payment-edit') || this.$can('invoice-payment-view') || this.$can('invoice-payment-delete');
    },
    ...
    exportUrl() {
      ...
      return `/payments/clients/invoice/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `client-invoice-payments-${(new Date()).toISOString().slice(0,10)}.pdf`,
