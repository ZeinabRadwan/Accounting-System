      ...
      <div class="col-lg-12">
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <GeneralTable
              :columns="nonInvoicePaymentColumns"
              :rows="nonInvoicePaymentsWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
            ...
            >
              ...
              <template #cell-paymentType="{ row }">
                <span v-if="parseInt(row.type) === 1" class="badge bg-primary">{{ $t('Payment Received') }}</span>
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('non-invoice-payment-view')">
                        <router-link :to="{ name: 'nonInvoicePayments.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('non-invoice-payment-edit') && row.status !== 2">
                        <router-link :to="{ name: 'nonInvoicePayments.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('non-invoice-payment-delete') && row.status === 1">
                      ...
                      <li v-if="$can('non-invoice-payment-delete') && row.status === 0">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('non-invoice-payment-edit') || this.$can('non-invoice-payment-show') || this.$can('non-invoice-payment-delete');
    },
    nonInvoicePaymentColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "client", label: this.$t("Client") },
        { key: "paymentType", label: this.$t("Payment Type") },
    ...
    exportUrl() {
      ...
      return `/payments/clients/non-invoice/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `client-non-invoice-payments-${(new Date()).toISOString().slice(0,10)}.pdf`,
...
.non-invoice-payments-table thead th {
...
.non-invoice-payments-table thead tr {
  border: none !important;
}

.non-invoice-payments-table thead th:first-child {
  border-top-left-radius: 10px;
}

.non-invoice-payments-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .non-invoice-payments-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .non-invoice-payments-table thead th:last-child {
...
.non-invoice-payments-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.non-invoice-payments-table .badge.bg-danger {
...
.non-invoice-payments-table .badge.bg-primary {
