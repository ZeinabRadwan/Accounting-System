      ...
      <div class="col-lg-12">
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <GeneralTable
              :columns="receiveVoucherColumns"
              :rows="receiveVouchersWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
            ...
            >
              ...
              <template #cell-amount="{ row }">
                <span v-html="formatCurrency(row.amount)"></span>
              </template>
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('payment-voucher-view')">
                        <router-link :to="{ name: 'receiveVouchers.show', params: { slug: row.slug } }">
                          <i class="fas fa-eye"></i>
                          {{ $t('View') }}
                        </router-link>
                      </li>
                      <li v-if="$can('payment-voucher-edit')">
                        <router-link :to="{ name: 'receiveVouchers.edit', params: { slug: row.slug } }">{{ $t('Edit') }}</router-link>
                      </li>
                      <li v-if="$can('payment-voucher-delete')">
                        <a href="#" @click.prevent="deleteItem(row.slug)">{{ $t('Delete') }}</a>
                      </li>
                    </ul>
                  </div>
                </div>
...
export default {
  ...
  computed: {
    ...
    receiveVoucherColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "entityType", label: this.$t("Entity Type") },
        { key: "entityName", label: this.$t("Entity Name") },
        { key: "paymentMethod", label: this.$t("Payment Method") },
