      ...
      <div class="col-12 col-xl-9">
        <div class="card custom-card w-100">
          <div class="card-header setings-header">
            <h3 class="card-title">{{ $t("Analytical Accounts") }}</h3>
          </div>
          <!-- /.card-header -->
          <div class="card-body position-relative">
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  <router-link :to="{ name: 'analyticalAccounts.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            <GeneralTable
              :columns="analyticalAccountColumns"
              :rows="analyticalAccountsWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
              :show-actions="true"
            >
              <template #cell-status="{ row }">
                <span v-if="row.status === 1 || row.status === true" class="badge bg-success">{{ $t("Active") }}</span>
              ...
              <template #cell-branch="{ row }">
                <span v-if="row.branch">
                  {{ row.branch.name }}
                </span>
                <span v-else class="text-muted">{{ $t("Global") }}</span>
              </template>
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li>
                        <router-link :to="{ name: 'analyticalAccounts.edit', params: { id: row.id } }">
                          <i class="fas fa-edit"></i>
                          {{ $t('Edit') }}
                        </router-link>
                      </li>
                      <li>
                        <a href="#" @click.prevent="deleteData(row.id)">
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("Analytical Accounts") };
  },
  data: () => ({
  ...
  methods: {
    ...
    async deleteData(id) {
      Swal.fire({
        ...
        if (result.value) {
          axios.delete(`/api/analytical-accounts/${id}`)
            .then((response) => {
              if (response.data.success) {
                toast.fire({
                  type: "success",
                  title: this.$t("Deleted!"),
                  text: response.data.message || this.$t("Analytical account deleted successfully."),
                });
                this.getData();
              } else {
                toast.fire({
                  type: "warning",
                  title: this.$t("Failed!"),
                  text: response.data.message || this.$t("There was something wrong."),
                });
              }
            })
            .catch((error) => {
              const errorMessage = error.response?.data?.message || 
                                 ...
                                 this.$t("Failed to delete analytical account. It may be in use.");
              toast.fire({
                type: "error",
                title: this.$t("Error!"),
                text: errorMessage,
...
.analytical-accounts-table thead th {
...
.analytical-accounts-table thead tr {
  border: none !important;
}

.analytical-accounts-table thead th:first-child {
  border-top-left-radius: 10px;
}

.analytical-accounts-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .analytical-accounts-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .analytical-accounts-table thead th:last-child {
