  ...
  <div>
    ...
    <div class="row">
      ...
      <div class="col-12 col-xl-9">
        <form role="form" @submit.prevent="saveAnalyticalAccount" @keydown="form.onKeydown($event)">
          <div class="card">
            <div class="card-header setings-header">
              <div class="col-xl-4 col-4">
                <h3 class="card-title">
                  {{ $t("Create Analytical Account") }}
                </h3>
              </div>
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("Create Analytical Account") };
  },
  data: () => ({
  ...
  methods: {
    ...
    async saveAnalyticalAccount() {
      // Set branch_id in form (required)
      this.form.branch_id = this.form.branchId;

      await this.form
        .post(window.location.origin + "/api/analytical-accounts")
        .then(() => {
          toast.fire({
            type: "success",
            title: this.$t("Analytical Account added successfully"),
          });
        ...
        .catch((error) => {
          if (error.response && error.response.status === 422) {
            ...
            toast.fire({
              type: "error",
              title: this.$t("Error!"),
              text: error.response?.data?.message || this.$t("Failed to create analytical account."),
            });
          }
        });
    },
