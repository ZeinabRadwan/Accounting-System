                ...
                <div class="card custom-card w-100">
                    ...
                    <div class="card-body position-relative">
                        ...
                        <div class="table-responsive table-custom mt-3" id="printMe">
                            <table class="table accounting-periods-table">
                                <thead>
                                    <th>{{ $t('ID') }}</th>
                                    <th>{{ $t('Name') }}</th>
                                    <th>{{ $t('Fiscal Year') }}</th>
                                ...
                                <tbody>
                                    <tr v-for="period in filteredPeriods" :key="period.id">
                                        ...
                                        <td class="text-right no-print">
                                            <div class="action-dropdown" :class="{ open: openActionIndex === period.id }">
                                                <button type="button" class="action-icon-btn" :data-action-index="period.id" @click.stop="toggleAction(period.id)">
                                                ...
                                                <div class="action-menu" v-if="openActionIndex === period.id">
                                                    ...
                                                    <ul>
                                                        <li>
                                                            <a href="#" @click.prevent="editPeriod(period)">
                                                                <i class="fas fa-edit"></i>
                                                                {{ $t('Edit') }}
                                                            </a>
                                                        </li>
                                                        <li v-if="!period.is_closed && period.id !== currentPeriodId">
                                                            <a href="#" @click.prevent="setCurrentPeriod(period)">
                                                                <i class="fas fa-check"></i>
                                                                {{ $t('Set as Current') }}
                                                            </a>
                                                        </li>
                                                        <li v-if="!period.is_closed">
                                                            <a href="#" @click.prevent="closePeriod(period)">
                                                                <i class="fas fa-lock"></i>
                                                                {{ $t('Close Period') }}
                                                            </a>
                                                        </li>
                                                        <li v-if="period.is_closed">
                                                            <a href="#" @click.prevent="reopenPeriod(period)">
                                                                <i class="fas fa-unlock"></i>
                                                                {{ $t('Reopen Period') }}
                                                            </a>
                                                        </li>
                                                        <li v-if="!period.is_closed">
                                                            <a href="#" @click.prevent="deletePeriod(period)">
...
.accounting-periods-table thead th {
...
.accounting-periods-table thead tr {
  border: none !important;
}

.accounting-periods-table thead th:first-child {
  border-top-left-radius: 10px;
}

.accounting-periods-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .accounting-periods-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .accounting-periods-table thead th:last-child {
...
.accounting-periods-table .badge.bg-danger {
...
.accounting-periods-table .badge.bg-secondary {
