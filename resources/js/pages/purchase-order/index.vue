          ...
          <div class="card-header setings-header">
            ...
            <div class="col-xl-8 col-8 float-right text-right">
              <div class="btn-group c-w-100">
                ...
                <a v-if="$can('purchase-order-edit') ||
                  $can('purchase-order-view') ||
                  $can('purchase-order-delete')
                  " href="/purchase-order/pdf" v-tooltip="$t('Export to PDF')" class="btn btn-secondary">
                ...
                <router-link v-if="$can('purchase-order-create')" :to="{ name: 'purchase-order.create' }" class="btn btn-primary">
                  {{ $t("Create") }}
              ...
              <table class="table">
                <thead>
                  <tr>
                    ...
                    <th>{{ $t("Net Total") }}</th>
                    <th>{{ $t("Status") }}</th>
                    <th v-if="$can('purchase-order-edit') ||
                      $can('purchase-order-view') ||
                      $can('purchase-order-delete')
                  ...
                  <tr v-show="items.length" v-for="(data, i) in items" :key="i">
                    ...
                    <td>
                      <router-link v-if="$can('purchase-order-view')" :to="{
                        ...
                        {{ data.purchase_no | withPrefix(prefix) }}
                      </router-link>
                      <span v-else>{{ data.purchase_no | withPrefix(prefix) }}</span>
                    </td>
                    <td>
                      <span v-if="data.po_date">{{
                        data.po_date | moment("Do MMM, YYYY")
                      }}</span>
                    </td>
                    <td>{{ data.supplier.name }}</td>
                    <td>{{ data.net_total | withCurrency }}</td>
                    <td>
                    ...
                    <td v-if="$can('purchase-order-edit') ||
                        $can('purchase-order-view') ||
                        $can('purchase-order-delete')
                        " class="text-right no-print">
                      <div class="btn-group">
                        <router-link v-if="$can('purchase-order-view')" v-tooltip="$t('View')" :to="{
                          name: 'purchase-order.show',
                          params: { slug: data.slug },
                        }" class="btn btn-primary btn-sm">
                          <i class="fas fa-eye" />
                        </router-link>
                        <router-link v-if="$can('purchase-order-edit')" v-tooltip="$t('Edit')" :to="{
                          name: 'purchase-order.edit',
                          params: { slug: data.slug },
                        }" class="btn btn-info btn-sm">
                          <i class="fas fa-edit" />
                        </router-link>
                        <a v-if="$can('purchase-order-delete')" v-tooltip="$t('Delete')" href="#"
