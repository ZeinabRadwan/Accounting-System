          ...
          <div class="card-body position-relative">
            ...
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('purchase-order-create')" :to="{ name: 'purchase-order.create' }"
            ...
            >
              <template #cell-purchaseNo="{ row }">
                <router-link v-if="$can('purchase-order-view')" :to="{
                  name: 'purchase-order.show',
                  params: { slug: row.slug },
                }">
                  {{ row.purchaseNo || row.poNumber || row.purchase_no | withPrefix(prefix) }}
                </router-link>
                <span v-else>{{ (row.purchaseNo || row.poNumber || row.purchase_no) | withPrefix(prefix) }}</span>
              </template>
              <template #cell-poDate="{ row }">
                <span v-if="row.poDate || row.po_date">{{ (row.poDate || row.po_date) | moment("Do MMM, YYYY") }}</span>
              </template>
              <template #cell-supplier="{ row }">
                {{ row.supplierName || (row.supplier && row.supplier.name) }}
              </template>
              <template #cell-subTotal="{ row }">
                {{ Number(row.subTotal || row.sub_total || 0).toFixed(2) }} <span class="saudi-riyal">ê</span>
              </template>
              <template #cell-discount="{ row }">
                {{ formatNumber(getOrderProductDiscount(row)) }} <span class="saudi-riyal">ê</span>
              </template>
              <template #cell-tax="{ row }">
                {{ Number(row.totalTax || row.total_tax || 0).toFixed(2) }} <span class="saudi-riyal">ê</span>
              </template>
              <template #cell-netTotal="{ row }">
                {{ Number(row.netTotal || row.net_total || row.calculated_total || 0).toFixed(2) }} <span class="saudi-riyal">ê</span>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('purchase-order-view') && row.due > 0 && row.status === 1">
                      ...
                      <li v-if="$can('purchase-order-view')">
                        <router-link :to="{ name: 'purchase-order.show', params: { slug: row.slug } }">
                          <i class="fas fa-eye"></i>
                          {{ $t('View') }}
                        </router-link>
                      </li>
                      <li v-if="$can('purchase-create')">
                        <router-link :to="{ name: 'purchases.create', query: { fromPurchaseOrder: row.slug } }">
                          <i class="fas fa-file-invoice"></i>
                          {{ $t('Create Purchase Invoice') }}
                        </router-link>
                      </li>
                      <li v-if="$can('purchase-order-edit')">
                        <router-link :to="{ name: 'purchase-order.edit', params: { slug: row.slug } }">
                          {{ $t('Edit') }}
                        </router-link>
                      </li>
                      <li v-if="$can('purchase-order-delete')">
          ...
          <div class="row">
            <div class="form-group col-md-6">
              ...
              <input type="text" class="form-control" readonly
                :value="parseFloat(form.selectedInvoice.invoiceTotal).toFixed(2)" />
            </div>
            <div class="form-group col-md-6">
              ...
              <input type="text" class="form-control" readonly
                :value="parseFloat(form.selectedInvoice.due).toFixed(2)" />
            </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4">
              <label for="paidAmount">{{ $t("Paid Amount") }}</label>
              <input type="number" step="0.01" class="form-control" :placeholder="$t('Enter an amount')" required
                min="0.01" v-model="form.paidAmount" :max="form.selectedInvoice.due" />
            </div>
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("Purchase Orders") };
  },
  components: {
    DateRangePicker,
    ToggleButton,
  ...
  computed: {
    ...
    showActions() {
      return this.$can('purchase-order-view') || this.$can('purchase-order-edit') || this.$can('purchase-order-delete');
    },
    purchaseOrderColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "purchaseNo", label: this.$t("Purchase Order No") },
        { key: "poDate", label: this.$t("PO Date") },
        { key: "supplier", label: this.$t("Supplier") },
        { key: "subTotal", label: this.$t("Subtotal") },
        { key: "discount", label: this.$t("Discount") },
        { key: "tax", label: this.$t("Tax") },
        { key: "netTotal", label: this.$t("Net Total") },
        { key: "status", label: this.$t("Status") },
      ];
    },
    purchaseOrdersWithIndex() {
  ...
  methods: {
    // Calculate total product discount for an order (same logic as show page)
    getOrderProductDiscount(order) {
      // Fallback to header-level discount if detailed lines are not present
      if (!order || !order.purchase_order_products || !Array.isArray(order.purchase_order_products)) {
        return Number(order.discount || 0);
      }

      const totalDiscount = order.purchase_order_products.reduce((sum, line) => {
        const lineTotal = Number(line.quantity || 0) * Number(line.purchase_price || 0);
        const discount = line.discount_type === 'percentage'
          ? (lineTotal * Number(line.discount || 0) / 100)
          : Number(line.discount_amount || 0);

        return Number((sum + (isNaN(discount) ? 0 : discount)).toFixed(2));
      }, 0);
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `purchase-orders-${moment().format("YYYY-MM-DD")}.pdf`,
    ...
    async savePayment() {
      await this.form
        .post(window.location.origin + "/api/purchase-order-pay")
        .then(() => {
          toast.fire({
            type: "success",
            title: this.$t("Purchase order payment added successfully"),
          });
          this.$router.push({
            name: "purchase-order.show",
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("You will not be able to return to this! This will delete the purchase order permanently."),
        type: "warning",
