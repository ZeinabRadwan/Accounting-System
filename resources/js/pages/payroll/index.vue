                        ...
                        <div class="action-menu" v-if="openActionIndex === i" @click.stop>
                          <ul>
                            <li v-if="$can('payroll-view')">
                              <router-link :to="{ name: 'payroll.show', params: { slug: data.slug } }">{{ $t('View') }}</router-link>
                            </li>
                            <li v-if="$can('payroll-edit')">
                              <router-link :to="{ name: 'payroll.edit', params: { slug: data.slug } }">{{ $t('Edit') }}</router-link>
                            </li>
...
export default {
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/payroll/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}`;
    },
  },
  watch: {
    // watch search data
