        ...
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <a href="payroll/pdf" v-tooltip="$t('Export to PDF')" class="btn export-pdf-btn"
                    title="Export to PDF">
            ...
            <GeneralTable
              ...
              :rows="payrollsWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
              :show-actions="showActions"
            >
              <template #cell-empName="{ row }">
                <router-link v-if="$can('employee-view') && row.employee" :to="{
                  name: 'employees.show',
                  params: { slug: row.employee.slug },
                }">
                  {{ row.employee.name }}
                </router-link>
                <span v-else>{{ row.employee && row.employee.name }}</span>
              </template>
              ...
              <template #cell-designation="{ row }">
                <span v-if="row.employee">{{ row.employee.designation }}</span>
              </template>
              <template #cell-totalPaid="{ row }">
                <span v-if="row.transaction">{{ row.transaction.amount }}<span class="saudi-riyal">ê</span></span>
              </template>
              <template #cell-account="{ row }">
                <span v-if="row.transaction && row.transaction.cashbook_account">
                  {{ row.transaction.cashbook_account.account_number }}
                </span>
              </template>
              <template #cell-salaryDate="{ row }">
                <span v-if="row.salaryDate">{{ row.salaryDate | moment("Do MMM, YYYY") }}</span>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('payroll-view')">
                        <router-link :to="{ name: 'payroll.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('payroll-edit')">
                        <router-link :to="{ name: 'payroll.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("Employee Payroll") };
  },
  components: {
    DateRangePicker,
    GeneralTable,
  ...
  computed: {
    ...
    payrollColumns() {
      return [
        ...
        { key: "empName", label: this.$t("Emp Name") },
        { key: "empID", label: this.$t("Emp ID") },
        { key: "designation", label: this.$t("Designation") },
        { key: "salaryMonth", label: this.$t("Salary Month") },
        { key: "totalPaid", label: this.$t("Total Paid") },
        { key: "account", label: this.$t("Account") },
        { key: "salaryDate", label: this.$t("Salary Date") },
    ...
    exportUrl() {
      ...
      return `/payroll/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
...
[dir="rtl"] .payroll-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .payroll-table thead th:last-child {
...
.payroll-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
