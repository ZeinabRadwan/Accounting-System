...
export default {
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/cashbook/transactions/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `cashbook-transactions-${(new Date()).toISOString().slice(0,10)}.pdf`,
...
[dir="rtl"] .transactions-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .transactions-table thead th:last-child {
...
.transactions-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.transactions-table .badge.bg-danger {
