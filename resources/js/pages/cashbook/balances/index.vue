          ...
          <div class="card-body position-relative">
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  <a @click="getData()" href="#" v-tooltip="$t('Refresh')" class="btn btn-success refresh-btn">
            ...
            >
              <template #cell-bankName="{ row }">
                <span v-if="row.account">{{ row.account.bankName }}</span>
              </template>
              ...
              <template #cell-type="{ row }">
                <span v-if="row.type == 0" class="badge bg-success">{{ $t("Add Balance") }}</span>
                <span v-else class="badge bg-danger">{{ $t("Remove Balance") }}</span>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li>
                        <router-link :to="{ name: 'balances.edit', params: { slug: row.slug } }">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('account-balance-edit') || this.$can('account-balance-delete');
    },
    balanceColumns() {
      return [
    ...
    exportUrl() {
      ...
      return `/cashbook/balance-adjustments/export/excel?term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `cashbook-balance-adjustments-${(new Date()).toISOString().slice(0, 10)}.pdf`,
...
.balances-table thead tr {
  border: none !important;
}

.balances-table thead th:first-child {
  border-top-left-radius: 10px;
}

.balances-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .balances-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .balances-table thead th:last-child {
...
.balances-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.balances-table .badge.bg-danger {
