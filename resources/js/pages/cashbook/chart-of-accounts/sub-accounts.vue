            ...
            >
              ...
              <template #cell-parentAccount="{ row }">
                <span v-if="row.parent">
                  <router-link v-if="$can('chart-of-account-view')" :to="{ name: 'chart-of-accounts.show', params: { slug: row.parent.code } }" class="text-primary">
...
export default {
  name: 'SubAccountsIndex',
  middleware: ["auth", "check-permissions"],
  metaInfo() {
    return { title: this.$t("Sub Accounts") };
  },
  components: {
    GeneralTable
  },
  data() {
    return {
      breadcrumbsCurrent: "Sub Accounts",
      breadcrumbs: [
        {
          name: "Dashboard",
          url: "home",
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/chart-of-accounts/sub-accounts/export/excel?term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    query: function (newQ) {
  ...
  methods: {
    async loadAccounts() {
      ...
      try {
        ...
        const params = {
          ...
          sub_accounts_only: true, // Filter to show only sub-accounts
        }
        ...
        this.$toast.error('Error loading sub-accounts')
      } finally {
        this.loading = false
      }
    },
            ...
            if (error.response && error.response.data) {
              ...
              if (!errorMessage && errorData.errors && errorData.errors.chart_of_account) {
                ...
                if (specificErrors.length > 0) {
                  errorMessage = this.$t(specificErrors[0])
                }
              }
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `sub-accounts-${moment().format("YYYY-MM-DD")}.pdf`,
