          ...
          <div class="card-body position-relative">
            ...
            >
              ...
              <template #cell-type="{ row }">
                <span class="badge bg-info">{{ getTranslatedName(row.type) }}</span>
              </template>
              <template #cell-parentAccount="{ row }">
                <span v-if="row.parent">
                  {{ row.parent.code }} - {{ getTranslatedName(row.parent) }}
                </span>
  ...
  computed: {
    ...
    chartOfAccountColumns() {
      return [
        ...
        { key: "parentAccount", label: this.$t("Parent Account") },
        { key: "status", label: this.$t("Status") },
      ];
    },
    chartOfAccountsWithIndex() {
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `chart-of-accounts-${moment().format("YYYY-MM-DD")}.pdf`,
