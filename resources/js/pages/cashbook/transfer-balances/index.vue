      ...
      <div class="col-lg-12" v-if="$can('account-transfer-balance-list') ||
        ...
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <GeneralTable
              :columns="transferBalanceColumns"
              :rows="transferBalancesWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
            ...
            >
              ...
              <template #cell-fromAccount="{ row }">
                <span v-if="row.fromAccount">{{ row.fromAccount.label }}</span>
              </template>
              <template #cell-toAccount="{ row }">
                <span v-if="row.toAccount">{{ row.toAccount.label }}</span>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('account-transfer-balance-view')">
                        <router-link :to="{ name: 'transferBalances.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('account-transfer-balance-edit') || this.$can('account-transfer-balance-view') || this.$can('account-transfer-balance-delete');
    },
    transferBalanceColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "reason", label: this.$t("Reason") },
        { key: "fromAccount", label: this.$t("From Account") },
        { key: "toAccount", label: this.$t("To Account") },
        { key: "amount", label: this.$t("Amount") },
        { key: "date", label: this.$t("Date") },
        { key: "status", label: this.$t("Status") },
      ];
    },
    transferBalancesWithIndex() {
    ...
    exportUrl() {
      ...
      return `/cashbook/transfer-balances/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    // export table to PDF (hide action column)
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `cashbook-transfer-balances-${(new Date()).toISOString().slice(0,10)}.pdf`,
...
.transfer-balances-table {
  border-collapse: separate;
  border-spacing: 0;
}

.transfer-balances-table thead th {
...
.transfer-balances-table thead tr {
  border: none !important;
}

.transfer-balances-table thead th:first-child {
  border-top-left-radius: 10px;
}

.transfer-balances-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .transfer-balances-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .transfer-balances-table thead th:last-child {
...
.transfer-balances-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.transfer-balances-table .badge.bg-danger {
