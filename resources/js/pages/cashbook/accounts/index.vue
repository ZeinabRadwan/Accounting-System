                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('account-view')">
                        <router-link :to="{ name: 'accounts.show', params: { slug: row.slug } }">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('account-view') || this.$can('account-edit') || this.$can('account-delete');
    },
    accountColumns() {
      return [
    ...
    exportUrl() {
      ...
      return `/accounts/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `accounts-${(new Date()).toISOString().slice(0,10)}.pdf`,
