...
export default {
  ...
  metaInfo() {
    return { title: this.$t('cashbook.Add Bank Account') }
  },
  data: () => ({
    form: new Form({
      bankName: '',
  ...
  computed: {
    ...
    breadcrumbsCurrent() {
      return this.$t("cashbook.Add Bank Account");
    },
  },
    ...
    async saveAccount() {
      ...
      await this.form
        ...
        .then(() => {
          toast.fire({
            type: 'success',
            title: this.$t('cashbook.Account added successfully'),
          })
          this.$router.push({ name: 'accounts.index' })
        })
        .catch(() => {
          toast.fire({ type: 'error', title: this.$t('cashbook.Opps...something went wrong') })
        })
    },
