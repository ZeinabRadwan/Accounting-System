  ...
  <div class="mb-50">
          ...
          <div class="card-body position-relative">
            ...
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('increment-create')" :to="{ name: 'increments.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            >
              <template #cell-image="{ row }">
                <a v-if="row.employee && row.employee.image" href="#" id="show-modal"
                  @click="previewModal(row.employee.image)">
                  <img :src="row.employee.image" class="rounded preview-sm" loading="lazy" />
                </a>
              ...
              <template #cell-name="{ row }">
                <router-link v-if="row.employee" :to="{
                  name: 'employees.show',
                  params: { slug: row.employee.slug },
                }">
                  {{ row.employee.name }}
                </router-link>
              </template>
              <template #cell-empID="{ row }">
                <span v-if="row.employee">{{ row.employee.empID | withPrefix(employeePrefix) }}</span>
              </template>
              ...
              <template #cell-basicSalary="{ row }">
                <span v-if="row.employee">{{ row.employee.salary }} <span class="saudi-riyal">ê</span></span>
              </template>
              <template #cell-incrementAmount="{ row }">
                {{ row.incrementAmount }} <span class="saudi-riyal">ê</span>
              </template>
              <template #cell-presentSalary="{ row }">
                <span v-if="row.employee">{{ row.employee.totalSalary }} <span class="saudi-riyal">ê</span></span>
              </template>
              <template #cell-incrementDate="{ row }">
                <span v-if="row.incrementDate">{{ row.incrementDate | moment("Do MMM, YYYY") }}</span>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('increment-view')">
                        <router-link :to="{ name: 'increments.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('increment-edit')">
                        <router-link :to="{ name: 'increments.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <Modal v-if="showModal" @close="previewModal()">
      ...
      <div class="w-100" slot="body">
        <img :src="loanImage" class="rounded img-fluid" loading="lazy" />
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("Employee Salary Increments") };
  },
  components: {
    DateRangePicker,
    GeneralTable,
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/increments/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
    showActions() {
      return this.$can('increment-edit') || this.$can('increment-view') || this.$can('increment-delete');
    },
    incrementColumns() {
      return [
        ...
        { key: "empID", label: this.$t("Emp ID") },
        { key: "reason", label: this.$t("Increment Reason") },
        { key: "basicSalary", label: this.$t("Basic Salary") },
        { key: "incrementAmount", label: this.$t("Increment Amount") },
        { key: "presentSalary", label: this.$t("Present Salary") },
        { key: "incrementDate", label: this.$t("Increment Date") },
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `employee-increments-${(new Date()).toISOString().slice(0,10)}.pdf`,
...
.increments-table thead tr {
  border: none !important;
}

.increments-table thead th:first-child {
  border-top-left-radius: 10px;
}

.increments-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .increments-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .increments-table thead th:last-child {
...
.increments-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.increments-table .badge.bg-danger {
