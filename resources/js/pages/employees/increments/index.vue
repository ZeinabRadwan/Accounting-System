            ...
            <div class="table-responsive table-custom mt-3" id="printMe">
              <table class="table increments-table">
                <thead>
                  ...
                  <th>{{ $t("Name") }}</th>
                  <th>{{ $t("Emp ID") }}</th>
                  <th>{{ $t("Increment Reason") }}</th>
                  <th>{{ $t("Basic Salary") }}</th>
                  <th>{{ $t("Increment Amount") }}</th>
                  <th>{{ $t("Present Salary") }}</th>
                  <th>{{ $t("Increment Date") }}</th>
                  <th>{{ $t("Status") }}</th>
                  <th v-if="$can('increment-edit') ||
                    $can('increment-view') ||
                ...
                <tbody>
                  <tr v-show="items.length" v-for="(data, i) in items" :key="i">
                    ...
                    <td>
                      <a v-if="data.employee && data.employee.image" href="#" id="show-modal"
                        @click="previewModal(data.employee.image)">
                        <img :src="data.employee.image" class="rounded preview-sm" loading="lazy" />
                      </a>
                    ...
                    <td>
                      <router-link v-if="data.employee" :to="{
                        name: 'employees.show',
                        ...
                        <div class="action-menu" v-if="openActionIndex === i">
                          ...
                          <ul>
                            <li v-if="$can('increment-view')">
                              <router-link :to="{ name: 'increments.show', params: { slug: data.slug } }">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                            ...
                            <li v-if="$can('increment-edit')">
                              <router-link :to="{ name: 'increments.edit', params: { slug: data.slug } }">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
