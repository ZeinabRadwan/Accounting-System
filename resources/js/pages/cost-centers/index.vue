      ...
      <div class="col-lg-12">
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <GeneralTable :columns="costCenterColumns" :rows="costCentersWithIndex" :loading="loading"
              table-id="printMe" wrapper-class="mt-3" :direction="$i18n.locale === 'ar' ? 'rtl' : 'ltr'"
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('cost-center-view')">
                        <router-link :to="{ name: 'cost-centers.show', params: { slug: row.id } }">
                          <i class="fas fa-eye"></i>
                          {{ $t('View') }}
                        </router-link>
                      </li>
                      <li v-if="$can('cost-center-edit')">
                        <router-link :to="{ name: 'cost-centers.edit', params: { slug: row.id } }">
                          <i class="fas fa-edit"></i>
                          {{ $t('Edit') }}
                        </router-link>
                      </li>
                      <li v-if="$can('cost-center-delete')">
                        <a href="#" @click.prevent="deleteItem(row.id)" :class="{ 'disabled': !row.can_delete }">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('cost-center-view') || this.$can('cost-center-edit') || this.$can('cost-center-delete');
    },
    costCenterColumns() {
      return [
    ...
    costCentersWithIndex() {
      return this.costCenters.map((item, index) => ({
        ...item,
    ...
    exportUrl() {
      ...
      return `/cost-centers/export/excel?term=${this.query}&locale=${locale}`;
    }
  },
  watch: {
    query: {
  ...
  methods: {
      ...
      if (result.value) {
        await this.$http
          ...
          .catch(() => {
            this.$toast.warning(
              this.$t('Failed!'),
              this.$t('Failed to delete cost center')
            )
          })
      }
    },
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `cost-centers-${moment().format("YYYY-MM-DD")}.pdf`,
...
.cost-centers-table thead tr {
  border: none !important;
}

.cost-centers-table thead th:first-child {
  border-top-left-radius: 10px;
}

.cost-centers-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .cost-centers-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .cost-centers-table thead th:last-child {
...
.cost-centers-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.cost-centers-table .badge.bg-danger {
