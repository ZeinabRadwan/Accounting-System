      ...
      <div class="col-lg-12">
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            <div class="row d-fex" style="justify-content: space-between">
              ...
              <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('product-sub-category-create')" :to="{ name: 'productSubCats.create' }" class="btn btn-primary">
                  {{ $t("Create") }}
            ...
            >
              <template #cell-category="{ row }">
                <span v-if="row.category">{{ row.category.name }} [
                  {{ row.category.code | withPrefix(catPrefix) }} ]
                </span>
              </template>
              <template #cell-code="{ row }">
                {{ row.code | withPrefix(subCatPrefix) }}
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('product-sub-category-edit')">
                        <router-link :to="{ name: 'productSubCats.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('product-sub-category-delete')">
...
export default {
  ...
  computed: {
    ...
    exportUrl() {
      ...
      return `/product-sub-categories/export/excel?term=${this.query}&locale=${locale}`;
    },
    showActions() {
      return this.$can('product-sub-category-edit') || this.$can('product-sub-category-delete');
    },
    subCategoryColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "category", label: this.$t("Category") },
        { key: "code", label: this.$t("Sub Category Code") },
        { key: "name", label: this.$t("Sub Category Name") },
        { key: "status", label: this.$t("Status") },
      ];
    },
    subCategoriesWithIndex() {
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `product-sub-categories-${(new Date()).toISOString().slice(0,10)}.pdf`,
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("All products for this category will be deleted!"),
        type: "warning",
...
.sub-categories-table thead tr {
  border: none !important;
}

.sub-categories-table thead th:first-child {
  border-top-left-radius: 10px;
}

.sub-categories-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .sub-categories-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .sub-categories-table thead th:last-child {
...
.sub-categories-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.sub-categories-table .badge.bg-danger {
