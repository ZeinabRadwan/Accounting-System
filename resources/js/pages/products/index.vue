  ...
  <div class="mb-50">
        ...
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <GeneralTable
              ...
              :rows="itemsWithIndex"
              :loading="loading"
              :show-actions="$can('product-edit') || $can('product-view') || $can('product-delete')"
            >
              <template #cell-index="{ value }">
                {{ value }}
              </template>
              <template #cell-itemType="{ row }">
                <span class="text-capitalize">{{ row.itemType }}</span>
              </template>
              ...
              <template #cell-subCategory="{ row }">
                <span v-if="row.subCategory">{{ row.subCategory.name }} [{{
              ...
              <template #cell-itemUnit="{ row }">
                {{ row.itemUnit ? row.itemUnit.code : '-' }}
              </template>
              <template #cell-sellingPrice="{ row }">
                <span v-if="row.discount > 0"><del>{{ row.regularPrice }}</del>
                  {{ calculateSellingPrice(row)  }} <span class="saudi-riyal">ê</span> ({{
                    row.discount
                  }}%)</span>
                <span v-else>{{ calculateSellingPrice(row)  }} <span class="saudi-riyal">ê</span>
                </span>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('product-view')">
                        <router-link :to="{ name: 'products.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('product-edit')">
                        <router-link :to="{ name: 'products.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <Modal class="pay-modal" v-if="showUploadCsvModal" @close="showUploadCsvModal = false">
      ...
      <div class="w-100" slot="body">
        ...
        <form role="form" @submit.prevent="saveCsv" @keydown="form.onKeydown($event)">
          <div class="row">
