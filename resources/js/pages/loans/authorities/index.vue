      ...
      <div class="col-lg-12">
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <GeneralTable :columns="authorityColumns" :rows="authoritiesWithIndex" :loading="loading" table-id="printMe"
              wrapper-class="mt-3" :show-actions="showActions">
              ...
              <template #cell-ccLimit="{ row }">
                {{ row.ccLimit }} <span class="saudi-riyal">ê</span>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('loan-authority-view')">
                        <router-link :to="{ name: 'authorities.show', params: { slug: row.slug } }">
                      ...
                      <li v-if="$can('loan-authority-edit')">
                        <router-link :to="{ name: 'authorities.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"
                            fill="none">
                            <path d="M11.5 1.5L14.5 4.5L5.5 13.5H2.5V10.5L11.5 1.5Z" stroke="#6B7280" stroke-width="1.5"
                              stroke-linecap="round" stroke-linejoin="round" />
                          </svg>
                          {{ $t('Edit') }}
                        </router-link>
                      </li>
                      <li v-if="$can('loan-authority-delete')">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('loan-authority-edit') || this.$can('loan-authority-view') || this.$can('loan-authority-delete');
    },
    authorityColumns() {
      return [
        ...
        { key: "contactNumber", label: this.$t("Contact Number") },
        { key: "ccLimit", label: this.$t("CC Limit") },
        { key: "status", label: this.$t("Status") },
      ];
    },
    authoritiesWithIndex() {
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `loan-authorities-${(new Date()).toISOString().slice(0, 10)}.pdf`,
...
.loan-authorities-table thead th {
...
.loan-authorities-table thead tr {
  border: none !important;
}

.loan-authorities-table thead th:first-child {
  border-top-left-radius: 10px;
}

.loan-authorities-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .loan-authorities-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .loan-authorities-table thead th:last-child {
...
.loan-authorities-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.loan-authorities-table .badge.bg-danger {
