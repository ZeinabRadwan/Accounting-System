        ...
        <div class="card custom-card w-100">
          ...
          <div class="card-body position-relative">
            ...
            <div class="row">
              ...
              <div class="col-xl-8 col-8 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('loan-payment-create')" :to="{ name: 'loanPayments.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            <GeneralTable
              ...
              :rows="loanPaymentsWithIndex"
              :loading="loading"
              table-id="printMe"
              wrapper-class="mt-3"
              :show-actions="showActions"
            >
              <template #cell-paymentRef="{ row }">
                <router-link v-if="$can('loan-payment-view')" :to="{
              ...
              <template #cell-loanRef="{ row }">
                <span v-if="row.loan">{{ row.loan.reference }}</span>
              </template>
              <template #cell-authority="{ row }">
                {{ row.authority && row.authority.name }}
              </template>
              <template #cell-payable="{ row }">
                <span v-if="row.loan">{{ row.loan.payable }} <span class="saudi-riyal">ê</span></span>
              </template>
              <template #cell-amountPaid="{ row }">
                {{ row.amount }} <span class="saudi-riyal">ê</span>
              </template>
              <template #cell-interest="{ row }">
                <span v-if="row.interest">{{ row.interest }} <span class="saudi-riyal">ê</span></span>
              </template>
              <template #cell-account="{ row }">
                <span v-if="row.account">{{ row.account.accountNumber }}</span>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('loan-payment-view')">
                        <router-link :to="{ name: 'loanPayments.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('loan-payment-edit')">
                        <router-link :to="{ name: 'loanPayments.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('loan-payment-delete')">
...
export default {
  ...
  metaInfo() {
    return { title: this.$t("Loan Payments") };
  },
  components: {
    DateRangePicker,
    GeneralTable,
  ...
  computed: {
    ...
    showActions() {
      return this.$can('loan-payment-edit') || this.$can('loan-payment-view') || this.$can('loan-payment-delete');
    },
    loanPaymentColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "paymentRef", label: this.$t("Payment Ref.") },
        { key: "loanRef", label: this.$t("Loan Ref.") },
        { key: "authority", label: this.$t("Authority") },
        { key: "payable", label: this.$t("Payable") },
        { key: "amountPaid", label: this.$t("Amount Paid") },
    ...
    exportUrl() {
      ...
      return `/loan-payments/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `loan-payments-${(new Date()).toISOString().slice(0,10)}.pdf`,
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("You will not be able to return to this! This will delete the loan payment permanently."),
        type: "warning",
...
.loan-payments-table thead th:first-child {
  border-top-left-radius: 10px;
}

.loan-payments-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .loan-payments-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .loan-payments-table thead th:last-child {
...
.loan-payments-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.loan-payments-table .badge.bg-danger {
