          ...
          <div class="card-body position-relative">
            ...
            >
              ...
              <template #cell-authority="{ row }">
                <span v-if="row.authority">{{ row.authority.name }}</span>
              </template>
              ...
              <template #cell-interest="{ row }">
                {{ row.interestAmount }} <span class="saudi-riyal">ê</span>
                <span v-if="row.loanType == 1">({{ row.interestRate }}%)</span>
              </template>
              ...
              <template #cell-installment="{ row }">
                <p>
                  {{ row.perMonth }} <span class="saudi-riyal">ê</span> {{ $t('Per Month') }} X {{ row.duration }}
                </p>
              </template>
              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('loan-view')">
                        <router-link :to="{ name: 'loans.show', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <li v-if="$can('loan-edit')">
                        <router-link :to="{ name: 'loans.edit', params: { slug: row.slug } }">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('loan-edit') || this.$can('loan-view') || this.$can('loan-delete');
    },
    loanColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "reference", label: this.$t("Reference No") },
    ...
    exportUrl() {
      ...
      return `/loans/export/excel?start_date=${this.dateRange.startDate}&end_date=${this.dateRange.endDate}&term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `loans-${(new Date()).toISOString().slice(0, 10)}.pdf`,
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("You will not be able to return to this! This will delete the loan permanently."),
        type: "warning",
