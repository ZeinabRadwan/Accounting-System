              ...
              <template #actions="{ row, index }">
                <div class="action-dropdown" :class="{ open: openActionIndex === index }">
                  ...
                  <div class="action-menu" v-if="openActionIndex === index">
                    ...
                    <ul>
                      <li v-if="$can('asset-type-edit')">
                        <router-link :to="{ name: 'assetTypes.edit', params: { slug: row.slug } }">
                          <i class="fas fa-edit"></i>
                          {{ $t('Edit') }}
                        </router-link>
                      </li>
                      <li v-if="$can('asset-type-delete')">
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('asset-type-edit') || this.$can('asset-type-delete');
    },
    assetTypeColumns() {
      return [
    ...
    exportUrl() {
      ...
      return `/asset-types/export/excel?term=${this.query}&locale=${locale}`;
    },
  },
  watch: {
    // watch search data
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `asset-types-${(new Date()).toISOString().slice(0, 10)}.pdf`,
    ...
    async deleteData(slug) {
      Swal.fire({
        ...
        text: this.$t("You will not be able to return to this! This will delete the asset type permanently."),
        type: "warning",
...
.asset-types-table thead tr {
  border: none !important;
}

.asset-types-table thead th:first-child {
  border-top-left-radius: 10px;
}

.asset-types-table thead th:last-child {
  border-top-right-radius: 10px;
}

/* RTL styles for Arabic language */
[dir="rtl"] .asset-types-table thead th:first-child {
  border-top-left-radius: 0;
  border-top-right-radius: 10px;
}

[dir="rtl"] .asset-types-table thead th:last-child {
...
.asset-types-table .badge.bg-success {
  background: #F6FEF4 !important;
  color: #2AB930 !important;
...
.asset-types-table .badge.bg-danger {
