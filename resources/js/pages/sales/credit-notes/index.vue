          ...
          <div class="card-body position-relative">
            <div class="row d-fex" style="justify-content: flex-end">
              ...
              <div class="col-xl-6 col-3 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('invoice-return-create')" :to="{ name: 'creditNotes.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            <div class="table-responsive table-custom mt-3">
              <GeneralTable
                ...
                :show-actions="true"
              >
                <template #cell-creditNoteNo="{ row }">
                  <router-link v-if="$can('invoice-return-view')" :to="{ name: 'creditNotes.show', params: { slug: row.slug } }">
                    {{ row.creditNoteNo }}
                  </router-link>
                  <span v-else>{{ row.creditNoteNo }}</span>
                </template>
                ...
                <template #actions="{ row }">
                  <router-link v-if="$can('invoice-return-view')" :to="{ name: 'creditNotes.show', params: { slug: row.slug } }" class="btn btn-sm btn-info">
...
export default {
  ...
  computed: {
    ...
    breadcrumbsCurrentComputed() {
      return this.$t("Credit Note");
    },
    columns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "creditNoteNo", label: this.$t("Credit Note No") },
        { key: "invoiceNo", label: this.$t("Reference Sales Invoice") },
