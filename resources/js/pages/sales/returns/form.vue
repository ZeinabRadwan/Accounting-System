...
export default {
  ...
  methods: {
    ...
    async handlePreSelection() {
      ...
      const clientSlug = this.$route.query.client

      if (!invoiceSlug) {
        return // No invoice to pre-select
      }
      ...
      try {
        ...
        const invoiceResponse = await axios.get(`/api/invoices/${invoiceSlug}`)
        ...
        let targetClientSlug = clientSlug
        ...
        const selectedClient = this.items.find(client => 
          client.slug === targetClientSlug || 
          client.id === targetClientSlug ||
          (invoiceData.client && (client.id === invoiceData.client.id || client.slug === invoiceData.client.slug))
        )
        ...
        // Wait a bit for invoices to load
        await this.$nextTick()
        ...
        if (this.clientInvoices && this.clientInvoices.length > 0) {
          ...
          selectedInvoice = this.clientInvoices.find(invoice => 
            invoice.slug === invoiceSlug || 
            invoice.id === invoiceSlug
          )

          // If not found, try by ID from invoiceData
          if (!selectedInvoice && invoiceData.id) {
            selectedInvoice = this.clientInvoices.find(invoice => invoice.id === invoiceData.id)
          }
          ...
          if (!selectedInvoice && invoiceData.invoiceNo) {
            selectedInvoice = this.clientInvoices.find(invoice => 
              ...
              invoice.invoiceNo === invoiceData.invoice_no
            )
          }
        }
        ...
        toast.fire({
          type: 'error',
          title: this.$t('Error'),
          text: this.$t('Failed to load invoice. Please try again.'),
        })
      }
    },
  ...
  async mounted() {
    ...
    await this.$nextTick()
    // Wait a bit more for clients to be fully loaded from the store
    await new Promise(resolve => setTimeout(resolve, 300))
    await this.handlePreSelection()
  },
}
</script>
