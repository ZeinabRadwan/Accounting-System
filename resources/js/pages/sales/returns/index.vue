            ...
            <div class="row d-fex" style="justify-content: flex-end">
              ...
              <div class="col-xl-6 col-3 float-right text-right">
                <div class="btn-group c-w-100">
                  ...
                  <router-link v-if="$can('invoice-return-create')" :to="{ name: 'invoiceReturns.create' }" class="btn btn-primary">
                    {{ $t("Create") }}
            ...
            <div class="table-responsive table-custom mt-3" id="printMe">
              ...
              >
                <template #cell-returnNo="{ row }">
                  ...
                  <span v-else>{{ row.returnNo | withPrefix(invoiceReturnPrefix) }}</span>
                </template>
                ...
                <template #cell-subtotal="{ row }">
                  <span v-html="formatCurrency(row.subtotal || 0)"></span>
                </template>
                    ...
                    <div class="action-menu" v-if="openActionIndex === index">
                      ...
                      <ul>
                        <li v-if="isSaudiArabia && row.status === 0">
                          <a href="#" @click="sendCreditNote(row)">
                            <i class="fas fa-paper-plane"></i>
                            {{ $t('Send Credit Note') }}
                          </a>
                        </li>
                        <li v-if="$can('invoice-return-view')">
                          <router-link :to="{ name: 'invoiceReturns.show', params: { slug: row.slug } }">
                            <i class="fas fa-eye"></i>
                            {{ $t('View') }}
                          </router-link>
                        </li>
                        <li v-if="$can('invoice-return-edit') && !(isSaudiArabia && row.status === 1)">
                          <router-link :to="{ name: 'invoiceReturns.edit', params: { slug: row.slug } }">{{ $t('Edit') }}</router-link>
                        </li>
                        <li v-if="$can('invoice-return-delete') && !(isSaudiArabia && row.status === 1)">
              ...
              <table class="table invoice-returns-table" v-if="items.length > 0">
                <tfoot>
                  <tr>
...
export default {
  ...
  computed: {
    ...
    showActions() {
      return this.$can('invoice-return-edit') || this.$can('invoice-return-view') || this.$can('invoice-return-delete');
    },
    invoiceReturnColumns() {
      return [
        { key: "index", label: this.$t("#"), sortable: false },
        { key: "returnNo", label: this.$t("Return No") },
  ...
  methods: {
    ...
    async exportToPDF() {
      try {
        ...
        const opt = {
          margin: [10, 10, 10, 10],
          filename: `invoice-returns-${moment().format("YYYY-MM-DD")}.pdf`,
