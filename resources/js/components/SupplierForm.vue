            ...
            <div class="row">
              <template v-if="form.country === 'SA'">
                <div class="form-group col-md-6">
                  ...
                  <v-select v-if="saudiCities.length > 0 && form.saudi_region" v-model="form.city"
                    :options="saudiCities" label="name" :reduce="option => option.name" :placeholder="$t('Select City')"
                    :searchable="true" :clearable="false" class="saudi-location-select"
                    :class="{ 'is-invalid': form.errors.has('city') }" :disabled="!form.saudi_region">
                    <template #option="{ name_ar, name_en }">
                      <div>{{ $i18n.locale === 'ar' ? name_ar : name_en }}</div>
                    </template>
                    <template #selected-option="{ name_ar, name_en }">
